...
# ── Patrones de detección de metadatos del reporte (Ajustados para Resiliencia) ──
...
# Regex elástico multilineal para absorber quiebres de tablas de metadatos
_GL_SCORE_RE = re.compile(
    r'(?:gl[-\s]?score|wallet[-\s]?risk[-\s]?score|risk[-\s]?score|score|puntuaci[o\u00f3]n|puntaje)'
    r'[\s\:\-\–\—\.\=]+(\d{1,3})(?:\s*/\s*100)?\b',
    re.IGNORECASE
)
...
# Captura el nivel incluso si viene acompañado de caracteres de control o celdas divididas
_GL_RISK_LEVEL_RE = re.compile(
    r'(?:risk\s*level|risk\s*category|nivel\s*de\s*riesgo|riesgo|nivel|status|estatus)[\s\:\-\–\—\.\=]+(critical|high|medium|low|cr[i\u00ed]tico|critico|alto|medio|bajo)\b'
    r'|\b(critical|high|medium|low|bajo|medio|alto|cr[i\u00ed]tico|critico)\s*risk\b',
    re.IGNORECASE
)
...
def _risk_level_from_score(score: int) -> str:
    """
    Sincroniza la evaluación con crypto_repo.py (Escala Global Ledger)
    100 = Seguro/Limpio, 0 = Contaminación Máxima
    """
    if score < 20:
        return "Crítico"
    if score < 40:
...
def parse_gl_pdf(pdf_bytes: bytes) -> dict:
    ...
    # ── Detección de metadatos desde texto libre (Optimizado para Resiliencia) ──
    ...
    gl_risk_level_text: Optional[str] = None
    report_date_detected: Optional[str] = None
    last_tx_date_detected: Optional[str] = None
    _gl_anchor: int = -1
    
    if full_text_pages:
        # Limpieza inicial de espacios duros de maquetado
        _full_text = "\n".join(full_text_pages).replace("\xa0", " ")
        _normalized_text = " ".join(_full_text.split())
        
        # 1. Búsqueda de Wallet Address
        _mw = _WALLET_RE.search(_normalized_text)
        if _mw:
            wallet_detected = _mw.group(1)
            
        # 2. Búsqueda de Score con etiqueta sobre texto colapsado (Corrige campo vacío)
        _ms = _GL_SCORE_RE.search(_normalized_text)
        if _ms:
            try:
                _sc = int(_ms.group(1))
        ...
        # Fallback: "47/100" fraccionario en texto colapsado
        if gl_score_detected is None:
            _mf = _GL_SCORE_FRACTION_RE.search(_normalized_text)
            if _mf:
                try:
        ...
        # Fallback histórico: proximidad de marcador físico
        _gl_anchor = _full_text.find("GL-Score")
        if _gl_anchor == -1:
            _gl_anchor = _full_text.lower().find("gl score")
        if gl_score_detected is None and _gl_anchor != -1:
        ...
        # 3. Búsqueda de Nivel de riesgo sobre texto colapsado (Corrige campo vacío)
        _ml = _GL_RISK_LEVEL_RE.search(_normalized_text)
        if _ml:
            ...
            gl_risk_level_text = _lvl_map.get(_raw_lvl)

        # 4. Extracción de Fechas e Indicadores Complementarios
    ...
    sof_candidates = [i for i in indicators if i["flow"] in ("sof", "mixed", "unknown") and i["total_pct"] > 0]
    uof_candidates = [i for i in indicators if i["flow"] in ("uof", "mixed", "unknown") and i["total_pct"] > 0]

    sof_top = max(sof_candidates, key=_score_key) if sof_candidates else None
    uof_top = max(uof_candidates, key=_score_key) if uof_candidates else (indicators[1] if len(indicators) > 1 else None)
    
    if sof_top and uof_top and sof_top["entity"] == uof_top["entity"] and len(indicators) > 1:
        uof_top = next((i for i in indicators if i["entity"] != sof_top["entity"]), uof_top)

    _risk_level_map = {"Crítico": "CRITICAL", "Alto": "HIGH", "Medio": "MEDIUM", "Bajo": "LOW", "Sin Datos": "UNKNOWN"}
    ...
    for ind in indicators:
        ...
        _types    = ["SoF"] if _flow == "sof" else ["UoF"] if _flow == "uof" else ["SoF", "UoF"]
    ...
    _residuals: list[dict] = []

    for item in _consolidated.values():
        item["percentage"] = round(item["percentage"], 6)
        if item["level"] in _HIGH_LEVELS or item["percentage"] >= _RELEVANCE_THRESHOLD:
            risk_exposure_list.append(item)
        else:
            _residuals.append(item)

    _order = {"CRITICAL": 0, "HIGH": 1, "MEDIUM": 2, "LOW": 3, "UNKNOWN": 4}
    risk_exposure_list.sort(key=lambda x: (_order.get(x["level"], 9), -x["percentage"]))

    _sof_total_pct = sum(r["percentage"] for r in _consolidated.values() if r["type"] == "SoF")
    _uof_total_pct = sum(r["percentage"] for r in _consolidated.values() if r["type"] == "UoF")
    _sof_total_amt = sum(r["amount"] for r in _consolidated.values() if r["type"] == "SoF")
    _uof_total_amt = sum(r["amount"] for r in _consolidated.values() if r["type"] == "UoF")
    ...
    # ── Mapeo e Inferencia Segura del Nivel GL Final ────────────────────────
    ...
    _nivel_map_lv = {"critical": "Crítico", "critico": "Crítico", "high": "Alto", "alto": "Alto", "medium": "Medio", "medio": "Medio", "low": "Bajo", "bajo": "Bajo"}
    if _gl_anchor != -1 and gl_score_detected is not None:
        ...
        if _nivel_inline:
            _gl_level_detected = _nivel_map_lv.get(_nivel_inline.group(1).lower(), _gl_level_detected)

    if _gl_level_detected is None and gl_score_detected is not None:
        _gl_level_detected = "Crítico" if gl_score_detected < 20 else "Alto" if gl_score_detected < 40 else "Medio" if gl_score_detected <= 60 else "Bajo"
    
    # Si todo método falla, se preserva como 'Sin Datos' de acuerdo a las convenciones de la UI
    if _gl_level_detected is None:
        _gl_level_detected = "Sin Datos"
    ...
    return {
        ...
        "gl_risk_level_text":    gl_risk_level_text,
    }
...
def _fallback_text_extraction(pdf_bytes: bytes, gl_scores: dict[str, int]) -> list[dict]:
    import io  # noqa: PLC0415
    raw_rows: list[dict] = []
    known_labels = sorted(gl_scores.keys(), key=len, reverse=True)
    try:
        with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
    ...
    for label in known_labels:
        pattern = re.compile(r"\b" + re.escape(label) + r"\b.*? (<?\s*[\d.,]+\s*%)", re.IGNORECASE)
        for m in pattern.finditer(full_text):
            pct = _clean_pct(m.group(1))
            if pct > 0:
                raw_rows.append({
                    "entity":         label,
                    "exposure_pct":   pct,
...
def generate_weekly_delta(parsed: dict, prev_snapshot: Optional[dict]) -> str:
    ...
    prev_score = int(prev_snapshot["gl_score"]) if prev_snapshot and prev_snapshot.get("gl_score") is not None else None
    ...
    if new_score is not None and prev_score is not None:
        ...
        if diff == 0:
            ...
            lines.append(f"GL Score aumentó {diff:+d} pts ({prev_score} → {new_score}): riesgo en escalada.")
        else:
            lines.append(f"GL Score mejoró {diff:+d} pts ({prev_score} → {new_score}): riesgo a la baja.")
    ...
    prev_cont = 0.0
    if prev_snapshot:
        prev_cont = float(prev_snapshot.get("sof_cont_total") or 0) + float(prev_snapshot.get("uof_cont_total") or 0)
    ...
    if prev_cont > 0 and new_cont > 0:
        ...
        if abs(diff_c) < 0.001:
            ...
            lines.append(f"Contaminación total aumentó {diff_c:+.4f}% ({prev_cont:.4f}% → {new_cont:.4f}%).")
        else:
            lines.append(f"Contaminación total disminuyó {diff_c:+.4f}% ({prev_cont:.4f}% → {new_cont:.4f}%).")
    elif new_cont > 0:
    ...
    if high > 0:
        lines.append(f"Se detectaron {high} indicadores Crítico/Alto. Indicador dominante: {top_e}.")
